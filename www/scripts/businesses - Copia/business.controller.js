	...
	function BusinessDetailsController(
		...
		function toggleFavorites() {
			...
			if (vm.business.isInFavorites) {
				...
				ionicToast.show('\'' + vm.business.name + '\' foi adicionado aos seus favoritos', 'bottom', false, 2000);
			} else {
				favoriteBusinessesService.removeFromFavorites(business.guid);
				ionicToast.show('\'' + vm.business.name + '\'foi removido dos seus favoritos', 'bottom', false, 2000);
			}
		}
		...
		function setDistanceToOrigin() {
			distanceService.getDistanceToOrigin(business.mapdata).then(function(distance) {
		...
		function getDirections() {
			externalAppsService.openMapsApp(business.mapdata);
		}
	}
